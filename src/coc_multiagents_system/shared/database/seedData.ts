...
import { CoCDatabase } from './schema.js';
...
export function seedDatabase(db: CoCDatabase): void {
    ...
    const count = database.prepare('SELECT COUNT(*) as count FROM skills').get() as { count: number };
    if (count.count > 0) {
...
function seedSkills(db: any): void {
    ...
    const skills = [
        ...
        ['Library Use', 20, 'Research in libraries, archives, databases', 'investigation', 0, null],
        ...
        ['Psychoanalysis', 1, 'Professional treatment of mental disorders', 'social', 1, null],
        ...
        ['Medicine', 1, 'Professional medical knowledge and practice', 'knowledge', 1, null],
        ['Natural World', 10, 'Knowledge of flora, fauna, and natural phenomena', 'knowledge', 0, null],
        ...
        ['Stealth', 20, 'Moving silently, hiding, avoiding detection', 'physical', 0, null],
        ...
        ['Disguise', 5, 'Changing appearance to avoid recognition', 'physical', 0, null],
        ['Drive Auto', 20, 'Driving automobiles', 'physical', 0, null],
        ['Electrical Repair', 10, 'Repairing electrical devices', 'knowledge', 0, null],
        ['First Aid', 30, 'Emergency medical treatment', 'knowledge', 0, null],
        ['Locksmith', 1, 'Picking locks and understanding security', 'physical', 1, null],
        ['Mechanical Repair', 10, 'Repairing mechanical devices', 'knowledge', 0, null],
        ['Navigate', 10, 'Finding direction, using maps', 'knowledge', 0, null],
        ['Operate Heavy Machinery', 1, 'Operating cranes, bulldozers, etc.', 'physical', 1, null],
        ['Sleight of Hand', 10, 'Pickpocketing, palming objects, stage magic', 'physical', 0, null],

        // Combat Skills
        ['Dodge', 0, 'Avoiding attacks and danger (calculated as DEX/2)', 'combat', 0, null],
        ...
        ['Cthulhu Mythos', 0, 'Knowledge of the Mythos (reduces max Sanity)', 'knowledge', 1, null]
    ];
...
function seedRules(db: any): void {
    ...
    const rules = [
        // Combat Rules
        ['combat_001', RuleCategory.COMBAT, 'Combat Overview',
         ...
         JSON.stringify(['A single gunshot can incapacitate or kill', 'Fleeing is often the wisest choice']),
         JSON.stringify(['combat', 'basic'])],

        ['combat_002', RuleCategory.COMBAT, 'Fighting Maneuvers',
         ...
         null, JSON.stringify(['combat', 'maneuvers'])],

        ['combat_003', RuleCategory.COMBAT, 'Attack Roll',
         ...
         null, JSON.stringify(['combat', 'attack'])],

        ['combat_004', RuleCategory.COMBAT, 'Dodge',
         ...
         null, JSON.stringify(['combat', 'defense', 'dodge'])],

        ['combat_005', RuleCategory.COMBAT, 'Fighting Back',
         ...
         null, JSON.stringify(['combat', 'defense', 'fighting back'])],

        ['combat_006', RuleCategory.COMBAT, 'Bonus Damage',
         ...
         null, JSON.stringify(['combat', 'damage', 'bonus'])],

        ['combat_007', RuleCategory.COMBAT, 'Hit Points',
         ...
         null, JSON.stringify(['combat', 'health', 'hit points'])],

        ['combat_008', RuleCategory.COMBAT, 'Major Wound',
         ...
         null, JSON.stringify(['combat', 'injury', 'major wound'])],

        ['combat_009', RuleCategory.COMBAT, 'Dying',
         ...
         null, JSON.stringify(['combat', 'death', 'dying'])],

        // Sanity Rules
        ['sanity_001', RuleCategory.SANITY, 'Sanity Points',
         ...
         null, JSON.stringify(['sanity', 'basic'])],

        ['sanity_002', RuleCategory.SANITY, 'Sanity Loss',
         ...
         null, JSON.stringify(['sanity', 'loss'])],

        ['sanity_003', RuleCategory.SANITY, 'Temporary Insanity',
         ...
         JSON.stringify(['Fainting', 'Fleeing in panic', 'Physical hysteria', 'Psychosomatic symptoms']),
         JSON.stringify(['sanity', 'temporary insanity'])],

        ['sanity_004', RuleCategory.SANITY, 'Indefinite Insanity',
         ...
         null, JSON.stringify(['sanity', 'indefinite insanity'])],

        ['sanity_005', RuleCategory.SANITY, 'Going Permanently Insane',
         ...
         null, JSON.stringify(['sanity', 'permanent insanity'])],

        ['sanity_006', RuleCategory.SANITY, 'Phobias and Manias',
         ...
         null, JSON.stringify(['sanity', 'phobia', 'mania'])],

        // General Rules
        ['general_001', RuleCategory.GENERAL, 'Success Levels',
         ...
         null, JSON.stringify(['basic', 'skill check'])],

        ['general_002', RuleCategory.GENERAL, 'Opposed Rolls',
         ...
         null, JSON.stringify(['basic', 'opposed'])],

        ['general_003', RuleCategory.GENERAL, 'Pushing Rolls',
         ...
         null, JSON.stringify(['advanced', 'push', 'skill check'])],

        ['general_004', RuleCategory.GENERAL, 'Bonus and Penalty Dice',
         ...
         null, JSON.stringify(['advanced', 'dice', 'bonus', 'penalty'])],

        ['general_005', RuleCategory.GENERAL, 'Luck',
         ...
         null, JSON.stringify(['advanced', 'luck'])],

        ['char_001', RuleCategory.CHARACTER, 'Characteristics',
         'Seven primary characteristics define a character.',
         'STR, CON, SIZ, DEX, APP, INT, POW, EDU. Roll 3d6 for most, 2d6+6 for INT and EDU.',
         null, JSON.stringify(['character creation', 'characteristics'])],

        ['char_002', RuleCategory.CHARACTER, 'Derived Attributes',
         ...
         null, JSON.stringify(['character creation', 'attributes'])]
    ];
...
function seedWeapons(db: any): void {
    ...
    const weapons = [
        ...
        ['.45 Revolver', 'Firearms (Handgun)', '1d10+2', '15 yards', 2, 6, 100, null],
        ['.45 Automatic', 'Firearms (Handgun)', '1d10+2', '15 yards', 3, 7, 100, null],
        ['9mm Automatic', 'Firearms (Handgun)', '1d10', '15 yards', 3, 15, 97, null],

        // Rifles & Shotguns
        ['.22 Rifle', 'Firearms (Rifle/Shotgun)', '1d6', '30 yards', 1, 6, 99, null],
        ['.30-06 Rifle', 'Firearms (Rifle/Shotgun)', '2d6+4', '110 yards', 1, 5, 100, null],
        ['Shotgun 12-gauge', 'Firearms (Rifle/Shotgun)', '4d6/2d6/1d6', '10/20/50 yards', 1, 2, 100, null],
        ['Shotgun 20-gauge', 'Firearms (Rifle/Shotgun)', '2d6/1d6/1d3', '10/20/50 yards', 1, 2, 100, null],

        // Submachine guns
        ['Thompson SMG', 'Firearms (Submachine Gun)', '1d10+2', '20 yards', 3, 20, 96, null],

        // Other
        ['Dynamite Stick', 'Throw', '4d10', '10 yards', 1, null, null, null],
        ['Molotov Cocktail', 'Throw', '2d6 (burn)', '10 yards', 1, null, null, null]
    ];
