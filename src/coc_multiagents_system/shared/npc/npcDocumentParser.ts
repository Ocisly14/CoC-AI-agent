...
import { ParsedNPCData } from '../models/gameTypes.js';
...
export class NPCDocumentParser {
    ...
    async parseDocument(filePath: string): Promise<ParsedNPCData> {
        ...
        if (!['docx', 'pdf'].includes(ext)) {
            throw new Error(`Unsupported file format: ${ext}. Only .docx and .pdf are supported.`);
        }
     ...
     */
    private async extractNPCData(text: string, fileName: string): Promise<ParsedNPCData> {
...
Return ONLY the JSON object, no additional text.`;
        ...
        if (!npcData.name) {
            throw new Error(`NPC name is required but not found in document: ${fileName}`);
        }
