...
export const MEMORY_AGENT_MANIFEST: AgentManifest = {
    ...
    purpose: 'Records game events, maintains searchable history, tracks discoveries, manages relationships, provides context for decisions',
    capabilities: [
        {
            name: 'logEvent',
            description: 'Records a game event to permanent log',
            parameters: [
                { name: 'eventType', type: 'string', description: 'Type of event (narration, action, roll, combat, dialogue, discovery)', required: true },
                { name: 'details', type: 'any', description: 'Event details', required: true },
                { name: 'timestamp', type: 'Date', description: 'When event occurred', required: true }
            ],
            returns: 'Event ID'
        },
        {
            ...
            parameters: [
                ...
                { name: 'filters', type: 'any', description: 'Event type, time range, etc.', required: false }
            ],
        ...
        },
        {
            ...
            parameters: [
                { name: 'depth', type: 'string', description: 'Context depth: recent, session, or campaign', required: true }
            ],
        ...
        },
        {
            ...
            parameters: [
                { name: 'sessionId', type: 'string', description: 'Session to summarize', required: true }
            ],
        ...
        },
        {
            ...
            parameters: [
                ...
                { name: 'npcId', type: 'string', description: 'NPC ID', required: true },
                { name: 'change', type: 'number', description: 'Relationship change (-10 to +10)', required: true }
            ],
        ...
        },
        {
            ...
            parameters: [
                ...
                { name: 'discoverer', type: 'string', description: 'Character who found it', required: true },
                { name: 'method', type: 'string', description: 'How it was discovered', required: true }
            ],
