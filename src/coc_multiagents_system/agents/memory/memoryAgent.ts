...
export class MemoryAgent {
     ...
     */
    public getContext(sessionId: string, depth: 'recent' | 'session' | 'campaign'): GameEvent[] {
        let limit = 10;
     ...
     */
    public getRelationship(characterId: string, npcId: string, sessionId: string): Relationship | null {
     ...
     */
    public searchLogs(keyword: string, sessionId?: string, limit: number = 50): GameEvent[] {
        let sql = `
     ...
     */
    public getRecentEvents(sessionId: string, count: number = 10): GameEvent[] {
        return this.queryHistory({
            sessionId,
            limit: count
        });
     ...
     */
    public getCharacterActions(sessionId: string, characterId: string): GameEvent[] {
        return this.queryHistory({
            sessionId,
            characterId,
            eventType: 'action'
    ...
    public getStats(): {
        ...
        const events = this.db.prepare('SELECT COUNT(*) as count FROM game_events').get() as { count: number };
        const sessions = this.db.prepare('SELECT COUNT(*) as count FROM sessions').get() as { count: number };
        const discoveries = this.db.prepare('SELECT COUNT(*) as count FROM discoveries').get() as { count: number };
    ...
    public getSkill(skillName: string): Skill | undefined {
        const row = this.db.prepare('SELECT * FROM skills WHERE name = ?').get(skillName) as any;
    ...
    public getSkillsByCategory(category: string): Skill[] {
        const rows = this.db.prepare('SELECT * FROM skills WHERE category = ?').all(category) as any[];
        return rows.map(row => ({
            name: row.name,
            baseValue: row.base_value,
            description: row.description,
    ...
    public getWeapon(weaponName: string): WeaponData | undefined {
        const row = this.db.prepare('SELECT * FROM weapons WHERE name = ?').get(weaponName) as any;
        if (!row) return undefined;
    ...
    public async lookupRule(query: RuleQuery): Promise<RuleLookupResult> {
        ...
        if (query.keywords && query.keywords.length > 0) {
            const keywordConditions = query.keywords.map(() => '(title LIKE ? OR description LIKE ? OR mechanics LIKE ?)').join(' OR ');
        ...
        const rules: RuleEntry[] = results.map(row => ({
            ...
            examples: row.examples ? JSON.parse(row.examples) : undefined,
            relatedRules: row.related_rules ? JSON.parse(row.related_rules) : undefined,
            tags: row.tags ? JSON.parse(row.tags) : undefined
        }));
    ...
    public getSanityLoss(trigger: string): string | undefined {
        const result = this.db.prepare('SELECT sanity_loss FROM sanity_triggers WHERE trigger = ?').get(trigger) as { sanity_loss: string } | undefined;
    ...
    public getAllSanityTriggers(): Map<string, string> {
        const results = this.db.prepare('SELECT trigger, sanity_loss FROM sanity_triggers').all() as Array<{ trigger: string, sanity_loss: string }>;
    ...
    public async skillCheck(
        ...
        const description = this.formatSkillCheckResult(skillName, roll, target, level, difficulty);
    ...
    public async sanityCheck(check: SanityCheck): Promise<SanityLossResult> {
        ...
        if (indefiniteInsanity) {
            effect = 'Indefinite Insanity - character develops a permanent phobia or mania and requires psychiatric treatment';
        } else if (permanentInsanity) {
            effect = 'Permanent Insanity - character is permanently insane and removed from play';
        } else if (temporaryInsanity) {
            effect = 'Temporary Insanity - character is incapacitated for 1d10+4 rounds or longer. Roll on Temporary Insanity table';
        }
     ...
     */
    public calculateMoveRate(dex: number, str: number, siz: number, age: number): number {
        let moveRate = 8;
     ...
     * Roll damage
     */
    public rollDamage(formula: string, bonus: number = 0): DamageResult {
    ...
    private rollD100WithBonusPenalty(bonusDice: number = 0, penaltyDice: number = 0): number {
        ...
        let result = tensDie * 10 + unitsDie;
