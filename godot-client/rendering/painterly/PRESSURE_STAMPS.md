...
- **贴地物体以背光侧接地轮廓为可见力度起点**，避免大建筑把整个压开过程遮在自身下面。停止在建筑下面整段盖印，只有背光侧落笔的完整圆头少量埋入建筑；悬空物体按投影起点行笔，不产生虚假的接地补色。
- 基础笔宽决定排笔中心。每次盖印的宽度由力度响应从 0.45 倍变到 1.2 倍；压力同时缩短笔尖沿行笔方向的接触长度。默认倾角由 50°过渡到收笔的 80°，辅助改变接触形状。
- Shape 为完整椭圆笔尖，原始颗粒让边缘与内部出现不规则露底。包内没有附带内置 Shape 原图，因此程序轮廓有明确标记，没有把 QuickLook 预览图当作笔尖。
...
- 最多搜索 5 条相邻笔触通道，横向裁剪扩大至基础笔宽的 ±0.71；公开宽度上限约束在 1.35 倍，使最大形变与搜索范围一致。80%–120% 控制路径长度，纵向允许完整笔尖伸出路径端点，最大预留 0.55 倍基础笔宽；不再把首尾印记压扁或做 UV 0–1 硬裁切。

输出继续进入覆盖、积色和干刷通道，只在原有直接日照合成里使用一次阴影可见度。接地加深只作用于已存在的笔刷覆盖，因此将力度或含漆量调到 0 时，笔触及其接地加深一起消失，不再留下独立矩形暗块。
...
28 项压力检查通过，包括 1,116 个根部点保持相连并允许边缘部分覆盖、846 个可见地面点物理 bias 隔离误差为 0、不同接收高度／光向／种子、悬空物体、相机缩放、最大笔宽、资源隔离及曲线更新。间距从 10% 改为 8% 时平均覆盖差约 0.0080；延长笔触后的同世界位置纹理平均差约 0.0034。大建筑可见起点回归、零力度和零含漆量也分别检查。旧整笔贴图路径 93 项、上一版 Aui 路径 18 项为此前记录，不作为本次起笔修复验证。最新修复检查见 [SHADOW_ROOT_FIX.md](SHADOW_ROOT_FIX.md)。
