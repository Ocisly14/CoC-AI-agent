extends RefCounted
## Scene-wide geometric seam compiler. No asset names, roof/wall assumptions,
## screen buffers or recursive colour reads. Compiled stamps live on receivers.
...
func rebuild(renderer: Node, records: Array) -> Dictionary:
	...
	stats={"build_count":build_count,"build_ms":(Time.get_ticks_usec()-start)/1000.0,"eligible_surfaces":registry.size(),"seams":seams.size(),"receivers":receivers,"stamps":stamps.size(),"drips":mini(drip_candidates.size(),renderer.seam_max_drips),"seam_length_m":total_length,"covered_length_m":covered_length,"coverage_upper_bound":covered_length/maxf(total_length,0.001),"errors":errors.duplicate()}
	_images.clear();_geometry.clear()
	return stats

func upload(record: Dictionary, transform: Transform3D, own: Array[Dictionary], renderer: Node) -> void:
	var origins:=PackedVector4Array();var tangents:=PackedVector4Array();var directions:=PackedVector4Array()
	var colors:=PackedVector4Array();var styles:=PackedVector4Array();var drips:=PackedVector4Array()
	for stamp in own:
		origins.append(Vector4(stamp.center.x,stamp.center.y,stamp.center.z,stamp.width))
		tangents.append(Vector4(stamp.tangent.x,stamp.tangent.y,stamp.tangent.z,stamp.reach))
		directions.append(Vector4(stamp.inward.x,stamp.inward.y,stamp.inward.z,stamp.opacity))
		colors.append(Vector4(stamp.color.r,stamp.color.g,stamp.color.b,1.0))
		styles.append(Vector4(stamp.variant,stamp.tilt,stamp.drip_length,stamp.drip_width))
		drips.append(Vector4(stamp.drip_direction.x,stamp.drip_direction.y,stamp.drip_direction.z,0.0))
	for array in [origins,tangents,directions,colors,styles,drips]: array.resize(MAX_STAMPS)
	var mat: ShaderMaterial=record.material
	mat.set_shader_parameter("seam_from_local",transform)
	mat.set_shader_parameter("seam_brushes",brush_layers)
	mat.set_shader_parameter("seam_regions",brush_regions)
	mat.set_shader_parameter("seam_origins",origins);mat.set_shader_parameter("seam_tangents",tangents)
	mat.set_shader_parameter("seam_directions",directions);mat.set_shader_parameter("seam_colors",colors)
	mat.set_shader_parameter("seam_styles",styles);mat.set_shader_parameter("seam_drips",drips)
	mat.set_shader_parameter("seam_stamp_count",own.size())
