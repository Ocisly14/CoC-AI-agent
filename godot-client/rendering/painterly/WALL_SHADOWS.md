...
- [墙面检查](../../demos/bluebird/qa/wall-shadow/validation.json)：171 项通过，其中 124 项逐表面检查接入状态，47 项检查场景覆盖与行为。新增的 8 项覆盖两种排笔模式的根部裁切：无真实遮挡时不能冒出根部、越过边界的根部像素为零、边界内保留笔触、裁切不能填成物理剪影。其余包括实际灯具物理／油画遮罩差异、投影开关，以及正墙、侧墙、25° 斜面的投影、相机稳定和光向变化。修复前 4 项根部防漏检查失败，修复后通过。
