...
此前蓝鸟适配器将立墙的 `painterly_shadows` 关闭，表面 Shader 又将压力／贴图投影限定为世界水平面。现在已接入 NPR 的建筑表面统一允许笔刷阴影，接收面法线决定投影坐标；控制图的 `map_plane` 继续只负责重要度、流向和间接光，不再阻止垂直面接收投影。

地面保留原坐标与缓存排笔。墙面及斜面在 Shader 中将光向和遮挡体积转换到接收面坐标，复用相同的压力、圆头、颗粒、接地和收笔过程。矩形排笔按该平面的米制投影宽度重新计算笔带数量，使用固定种子及重叠范围，并支持反转绘画顺序；不复用只适用于地面的窄灯杆笔带数量。没有新增网格、深度捕获或 CPU 每帧排笔。
...
- [墙面检查](../../demos/bluebird/qa/wall-shadow/validation.json)：163 项通过，其中 124 项逐表面检查接入状态，39 项检查场景覆盖与行为。包括实际灯具物理／油画遮罩差异、灯具投影开关，以及正墙、侧墙、25° 斜面上的普通／矩形压力笔刷。移动相机按相同世界点重投影比较；转光源检查投影移动；零压力和背光检查无独立笔刷贡献。
