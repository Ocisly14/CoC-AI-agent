...
bpy.ops.wm.open_mainfile(filepath=str(BASE/'shell-v6/bluebird_painterly.blend'))
...
# Shared atlas only for the small roof fittings; the roof uses its existing BakeUV.
bpy.ops.object.select_all(action='DESELECT')
for o in parts:
    o.hide_set(False);o.select_set(True);uv=o.data.uv_layers.new(name='RoofBakeV7');o.data.uv_layers.active=uv
bpy.context.view_layer.objects.active=parts[0];bpy.ops.object.mode_set(mode='EDIT');bpy.ops.mesh.select_all(action='SELECT')
bpy.ops.uv.smart_project(angle_limit=math.radians(66),island_margin=.012,correct_aspect=True)
bpy.ops.object.mode_set(mode='OBJECT')
for o in parts:o.data.uv_layers.active=o.data.uv_layers['UVMap']
(ROOT/'source/placements.json').write_text(json.dumps({'base':'v6 full building; original charcoal roof texture retained','coordinates':'world X/Y with upward-facing normal and height gates','patches':patches,'mainTone':'charcoal graphite','relatedTones':['slate blue-grey','muted grey-violet','olive-grey','warm graphite'],'tintMix':.82},ensure_ascii=False,indent=2))
scene['paintRevision']='v7 roof: charcoal base with related tonal pigment patches'
...
for cat,size,uvname in [('roof',4096,'BakeUV'),('flashing',2048,'RoofBakeV7')]:
    mats=[m for m in modified if m.get('v7_target')==cat];copies=[]
    image=bpy.data.images.new('V7_BAKED_'+cat,width=size,height=size,alpha=False);image.colorspace_settings.name='sRGB'
    for name in names:
        ...
        used=sorted({p.material_index for p in dup.data.polygons});keep=[dup.data.materials[i] for i in used];indices=[used.index(p.material_index) for p in dup.data.polygons];dup.data.materials.clear()
    ...
    for m in mats:
        tx=m.node_tree.nodes.new('ShaderNodeTexImage');tx.image=image;tx.name='V7_BAKE_TARGET';m.node_tree.nodes.active=tx
    bpy.ops.object.select_all(action='DESELECT')
    ...
    for m in mats:
        n=m.node_tree.nodes;l=m.node_tree.links;p=n.get('Principled BSDF');n.remove(n.get('V7_BAKE_TARGET'))
        uv=n.new('ShaderNodeUVMap');uv.uv_map=uvname;tx=n.new('ShaderNodeTexImage');tx.image=image;tx.extension='EXTEND';l.new(uv.outputs[0],tx.inputs['Vector']);l.new(tx.outputs['Color'],p.inputs['Base Color'])
    print('V7 BAKED',cat,flush=True)
