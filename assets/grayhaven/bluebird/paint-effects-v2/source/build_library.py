...
manifest=[]
families=['slate-scumbles','olive-graphite','warm-earth-scumbles']
tints=[['5C626A','62676B','575E63','65636B'],['66665B','6A685B','626252','716957'],['92816B','99816D','8D806F','968064']]
for row,family in enumerate(families):
    ...
    for col in range(4):
        ident=f'TONE_{row+1:02}_{col+1:02}'
        ...
        mask=n.new('ShaderNodeMapRange');mask.clamp=True;mask.inputs['From Min'].default_value=.008;mask.inputs['From Max'].default_value=.045
...
bpy.ops.object.camera_add(location=(5,4.8,20));cam=bpy.context.object;cam.rotation_euler=(0,0,0);cam.data.type='ORTHO';cam.data.ortho_scale=11.8;scene.camera=cam
...
(ROOT/'manifest.json').write_text(json.dumps({'version':2,'encoding':'sRGB source atlases; intentional black background; coverage derived in linear shader, .008 to .045','groups':manifest},ensure_ascii=False,indent=2))
bpy.ops.wm.save_as_mainfile(filepath=str(ROOT/'tonal-pigment-library.blend'))
